...
public class DirectMappingEngineWD20110920 implements DirectMappingEngine {
	 ...
	 */
	private String generateUniqNodeIRI(Row r, Table t, CandidateKey primaryKey, String baseURI) throws UnsupportedEncodingException{
	...
	private Statement convertLex(Header header, Row r, String columnName,
		...
		if (sqlType.isBlobType()) {
			...
			type = SQLToRDFToolkit.getEquivalentType(Integer.valueOf(d));
			if (type == null)
				throw new IllegalStateException(
	...
	private URI convertDatatype(String datatype) {
		...
		if (!SQLToRDFToolkit.isValidSQLDatatype(Integer.valueOf(upDatatype)))	
	    log.debug("[DirectMapper:convertDatatype] Unknown datatype : "
						+ datatype);
		String xsdDatatype = SQLToRDFToolkit.getEquivalentType(Integer.valueOf(upDatatype))
				.toString();
