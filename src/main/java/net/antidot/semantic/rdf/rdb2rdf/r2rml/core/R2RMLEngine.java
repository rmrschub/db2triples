...
public class R2RMLEngine {
	...
	private void extractRDFFromTriplesMap(SesameDataSet sesameDataSet,
		...
		HashSet<StdTriplesMap> triplesMaps = new HashSet<StdTriplesMap>(r2rmlMapping
				.getTriplesMaps());
		for (StdTriplesMap triplesMap : triplesMaps) {
			if (log.isDebugEnabled())
			...
			while (logicalTable.next()) {
				// Extract subject
				StdSubjectMap subjectMap = triplesMap.getSubjectMap();
				...
				for (StdPredicateObjectMap predicateObjectMap : triplesMap
		...
		if ((datatype == null) && (column != null)) {
			...
			for (int i = 1; i <= meta.getColumnCount(); i++) {
				if (meta.getColumnName(i).equals(column)) {
					...
					if (SQLToXMLS.isValidSQLDatatype(t))
						datatype = SQLToXMLS.getEquivalentType(t);
					else
						return null;
				}
			}
	...
	private Set<URI> extractGraphFromPredicateObjectMap(
			StdPredicateObjectMap predicateObjectMap) throws SQLException {
	 ...
	 * @throws SQLException
	 */
	private ResultSet constructLogicalTable(StdTriplesMap triplesMap)
			throws SQLException {
		ResultSet rs = null;
	 ...
	 * @throws SQLException
	 */
	private Set<URI> extractGraphFromSubjectMap(StdSubjectMap subjectMap)
			throws SQLException {
		HashSet<URI> result = new HashSet<URI>();
		// Check graph properties
	...
	private Resource extractSubjectFromSubjectMap(ResultSet logicalTable,
			StdSubjectMap subjectMap) throws SQLException {
		// Check properties
			...
			if (objectMap.getTermType().equals(TermType.BLANK_NODE))
				...
				if ((datatype != null) && XSDType.isDateType(datatype)) {
					...
					long timestamp = logicalTable.getTimestamp(column)
							.getTime();
					String timeZone = SQLConnector.getMySQLTimeZone(conn);
	...
	private Resource extractObjectFromRefObjectMap(RefObjectMap refObjectMap)
		...
		StdTriplesMap parentTriplesMap = refObjectMap.getParentTriplesMap();
